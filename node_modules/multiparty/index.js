...
Form.prototype.parse = function(req, cb) {
  ...
  if (cb) {
    ...
    var end = function (done) {
      ...
      var files = {},
          createOffset = function(obj, name,value){

              if(typeof(name) === "string" ){
                  if( name.indexOf("[") !== -1 ){
                      name = name.replace(/[\[\]]*$/g, "").split(/\[|\]\[/);
                      return createOffset(obj, name, value);
                  }
                  else{
              ...
              if( name.length == 0){
                  if(obj[offset] == undefined)
                    return obj[offset] = value;
                      ...
                      obj[offset].push(value);
                      return ;
                  }
              }

              if(obj[offset] !== undefined)
                  return createOffset(obj[offset], name, value);
              else{
                  obj[offset] = {};
                  return createOffset(obj[offset], name, value );
              }
    ...
    self.on('field', function(name, value) {
        createOffset(fields, name, value);
    });
